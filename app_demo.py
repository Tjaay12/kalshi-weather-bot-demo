...
def render_hero(summary):
    ...
    st.write(
        "This system identifies mispriced weather prediction markets by comparing model-estimated probability against market price."
    )
    st.caption("Built with Codex")

    st.info("PUBLIC DEMO MODE — Read-only simulation. No live trading or API keys exposed.")

    st.markdown("#### At a Glance")
    glance_cols = st.columns(4)
    glance_cols[0].metric("Win Rate", f"{summary['win_rate']:.2f}%")
    glance_cols[1].metric("Total PnL", f"${summary['total_pnl']:.2f}")
    glance_cols[2].metric("ROI", f"{summary['roi'] * 100:.2f}%")
    glance_cols[3].metric("Graded Trades", summary["total_trades"])
...
def render_key_insight():
    ...
    st.info(
        "Key Insight: Most profit in this demo comes from NO-side trades, suggesting that some weather buckets may be overpriced relative to forecast-based probabilities."
    )
